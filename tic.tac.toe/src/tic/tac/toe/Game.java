...
public class Game {
	...
	private static void writeLog(String log) {
		try {
			...
			FileWriter yazici = new FileWriter("tictactoe.txt", true); // true : append
			yazici.write(log);
			yazici.close();
	...
	public static void main(String[] args) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/YYYY HH:mm:ss"); // d: day , M: month, y : year
		...
		while (true) {// somebodywins ile oluşturulan şart true olduğu sürece döner
			...
			if (board[userChoice - 1].equals("" + userChoice)) { // "" + userChoice yazarak sayı olan değeri String veri
																	// tipine çevirdik
			...
			if (somebodywins == true) {
				System.out.println("Oyuncu kazandı."); // \t tab boşluğu
			...
			if (notXFoundIndex > -1) {
				...
				while (true) {
					int computerChoice = rnd.nextInt(9); // 0 sayısı dahil belirlenen sınır 9 sayısı hariç rastgele bir
															// sayı
															// oluşturur.
...
//		if((board[0] + board[1] + board[2]).equals(USER_WINNER) ||
//				(board[3] + board[4] + board[5]).equals(USER_WINNER) ||
//				(board[6] + board[7] + board[8]).equals(USER_WINNER)) {
...
//		}
	...
	private static int checkNearWinner(String[] board) {
		...
		// ilk satırda oyuncunun kazanacak durumu var mı?
		for (int i = 0; i < 3; i++) {
